...
void set_mpu(void)
{
	...
	REG(MPU_BASE + MPU_RBAR_OFFSET) = MPU_RBAR_VALUE(0x00000000,0);//Define the starting address of each region
	REG(MPU_BASE + MPU_RASR_OFFSET) = 
	MPU_RASR_VALUE(MPU_XN_DISABLE, MPU_AP_FULL_ACCESS, MPU_TYPE_FLASH, 0, MPU_REGION_SIZE_1MB);//MPU_XN_DISABLE is "allow"


	//set region 1: sram (0x20000000), 128KB, forbid execution, full access, enable all subregion
	REG(MPU_BASE + MPU_RBAR_OFFSET) = MPU_RBAR_VALUE(0x20000000,1);//Define the starting address of each region
	REG(MPU_BASE + MPU_RASR_OFFSET) = 
